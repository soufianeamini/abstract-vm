...
enum class CheckedArithmeticResult { SUCCESS, OVERFLOW, UNDERFLOW, INVALID };
...
template <class T> CheckedArithmeticResult checked_add(T *res, T a, T b) {
  ...
  return CheckedArithmeticResult::INVALID;
}
...
checked_arithmetic::checked_add<int8_t>(int8_t *res, int8_t a, int8_t b) {
  ...
  if (psnip_safe_int8_add(res, a, b)) {
    if ((a < 0) == (b < 0))
      return CheckedArithmeticResult::OVERFLOW;
    else
      return CheckedArithmeticResult::UNDERFLOW;
  }
  return CheckedArithmeticResult::SUCCESS;
}
